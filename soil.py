...
# set up data logging to /data/data_logger(x).txt the .txt will be added automatically
j = 1
...
# Check soil2 and run if soil is dryer than initial reading 
        reading2 = round(sensor_soil2.read_u16()*conversion_factor,1)
...
# create the data output for the sensor run and log to file.
    log = reading0, trigger0, reading1, trigger1, reading2, trigger2
    file.write(str(log)+"\n")
    file.flush
    utime.sleep(repeat)
